...
export const JobsView: React.FC<JobsViewProps> = ({ jobs, session, onSaveJob, onDeleteJob }) => {
  ...
  return (
    <div className="space-y-8 md:space-y-24 animate-in fade-in duration-1000 pb-24 md:pb-0">
      ...
      {isFormOpen && (
        <div className="bg-[#0a0a0b] p-5 md:p-16 rounded-[32px] md:rounded-[48px] border border-white/10 shadow-3xl animate-in zoom-in-95 duration-500 relative">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 md:gap-8">
        {jobs.map((job) => {
          ...
          return (
            <div key={job.id} onClick={() => setSelectedJob(job)} className={`bg-[#0a0a0b] border p-5 md:p-8 rounded-[32px] md:rounded-[48px] flex flex-col h-full group transition-all duration-500 cursor-pointer ${isMyJob ? 'border-white/40 shadow-xl' : 'border-white/10 hover:border-white/30'}`}>
      ...
      {selectedJob && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/90 backdrop-blur-3xl">
          <div className="bg-[#1a1d23] w-full max-w-xl rounded-[40px] md:rounded-[64px] border border-white/20 shadow-3xl p-6 md:p-14 relative overflow-y-auto max-h-[90vh] no-scrollbar">
