...
import { Video, Mail, Building, MapPin, TrendingUp, Clock, Key, User, Briefcase, Sparkles, Target, Calendar as CalendarIcon, Phone, Globe } from 'lucide-react';
...
const TextAreaField = ({ label, icon: Icon, placeholder, value, onChange, required = false }: any) => (
  <div className="space-y-3 group">
    ...
    <textarea 
      ...
      onChange={e => onChange(e.target.value)}
      placeholder={placeholder}
      className="w-full bg-white/[0.03] border border-white/5 p-6 rounded-[24px] text-white text-base font-medium outline-none focus:border-indigo-600 focus:bg-white/[0.06] transition-all placeholder:text-white/10 h-40 resize-none"
    />
  </div>
);
    ...
    <div className="space-y-12">
      {regStep === 1 && (
        <div className="space-y-10 animate-in fade-in slide-in-from-right-8 duration-500">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <InputField label="Годовой оборот (млн ₽)" icon={TrendingUp} placeholder="500" value={regData.turnover} onChange={(v: string) => setRegData({...regData, turnover: v})} required />
          ...
          <InputField label="Ниша экспертизы" icon={Briefcase} placeholder="IT / Маркетинг / Производство" value={regData.direction} onChange={(v: string) => setRegData({...regData, direction: v})} required />
      ...
      {regStep === 2 && (
        <div className="space-y-10 animate-in fade-in slide-in-from-right-8 duration-500">
          <TextAreaField 
            label="Мировоззрение и принципы" 
            icon={Sparkles} 
            placeholder="Что для вас действительно важно в бизнесе и в людях? Каких ценностей вы придерживаетесь?" 
          ...
          <TextAreaField 
            ...
            icon={Target} 
            placeholder="Какой энергообмен вам интересен? Что вы ищете в молодых талантах (идеи, драйв, свежий взгляд)?" 
          ...
          <div className="bg-indigo-600/5 p-8 rounded-[32px] border border-indigo-500/10 space-y-4">
             ...
             <p className="text-[9px] text-slate-600 font-bold uppercase tracking-widest px-2">Помогает талантам лучше понять вашу энергию еще до встречи.</p>
          </div>
        </div>
      )}
