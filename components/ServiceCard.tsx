...
export const ServiceCard: React.FC<ServiceCardProps> = ({ service, mentorName, onClick }) => {
  return (
    <div
      role="button"
      tabIndex={0}
      ...
      <div className="h-2/3 relative overflow-hidden shrink-0 pointer-events-none">
        {service.imageUrl ? (
          ...
          <div className="w-full h-full bg-gradient-to-br from-[#1a1d23] to-black flex items-center justify-center">
            <span className="text-white/5 font-black text-4xl md:text-6xl font-syne tracking-tighter uppercase">ЛОТ</span>
          </div>
        )}
        ...
        <div className="absolute bottom-6 left-6 md:left-8 flex items-center gap-3">
          ...
          <div className="space-y-0.5">
            <p className="text-[7px] font-black text-white/40 uppercase tracking-widest">МЕНТОР</p>
            <p className="text-[10px] font-bold text-white uppercase tracking-tight">{mentorName || 'Проверенный'}</p>
          </div>
        </div>
      </div>
        ...
        <div className="flex items-end justify-between pt-4 md:pt-6 border-t border-white/5">
          <div className="space-y-1">
            ...
            <div className="flex items-baseline gap-1">
              <span className="text-xl md:text-3xl font-black text-white font-syne tracking-tighter">{service.price.toLocaleString()}</span>
