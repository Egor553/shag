  ...
  return (
    <div className="grid grid-cols-1 gap-6">
      {pendingUsers.map((user) => (
        <div key={user.id} className="bg-[#0a0a0b] border border-white/5 p-5 md:p-10 rounded-[48px] flex flex-col lg:flex-row items-center justify-between gap-6 md:gap-10 group hover:border-indigo-500/30 transition-all duration-500">
          <div className="flex-1 space-y-8 w-full">
            <div className="flex items-center gap-4 md:gap-6">
              <div className="w-16 h-16 md:w-20 md:h-20 bg-white/5 rounded-[28px] flex items-center justify-center text-indigo-500 font-black text-2xl md:text-3xl uppercase font-syne border border-white/5">
                {user.name[0]}
              </div>
              <div className="space-y-1">
                <div className="flex items-center gap-3">
                  <h3 className="text-xl md:text-2xl font-black text-white uppercase font-syne tracking-tight">{user.name}</h3>
