...
export const RegistrationFlow: React.FC<RegistrationFlowProps> = ({
  ...
  return (
    <div className="min-h-screen w-full flex flex-col bg-[#1a1d23] font-['Inter']">
      {/* Header / Nav */}
      <div className="sticky top-0 z-50 bg-[#1a1d23]/80 backdrop-blur-xl p-4 md:p-6 border-b border-white/10 flex items-center justify-between">
        <button
          type="button"
        ...
        <div className="flex gap-1.5">
          {[1, 2, 3].map(s => (
            <div key={s} className={`h-1 rounded-full transition-all duration-500 ${s === regStep ? `w-6 md:w-8 bg-${accentColor}-500 shadow-[0_0_10px_rgba(99,102,241,0.5)]` : (s < regStep ? 'w-3 md:w-4 bg-emerald-500' : 'w-3 md:w-4 bg-white/10')}`} />
          ))}
        </div>
      </div>

      <div className="flex-1 flex flex-col p-4 md:p-12 lg:p-24 overflow-x-hidden">
